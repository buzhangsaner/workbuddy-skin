...
为 Windows 与 macOS 版 WorkBuddy 提供 15 套内置视觉方案。每套主题都会同步替换背景、左侧菜单、活动项、场景按钮、快捷按钮、输入框、发送按钮和整体色板；图片素材只作纯背景，不包含伪造菜单或文字。
...
- Windows 免安装版：双击 `WorkBuddy-Skin-Manager-Portable-0.6.2-x64.exe`。
- Windows 安装版：双击 `WorkBuddy-Skin-Manager-Setup-0.6.2-x64.exe`。
- macOS Intel：打开 `WorkBuddy-Skin-Manager-0.6.2-x64.dmg`，将应用拖入“应用程序”后打开。
- macOS Apple Silicon：打开 `WorkBuddy-Skin-Manager-0.6.2-arm64.dmg`，将应用拖入“应用程序”后打开。
...
- `Choose WorkBuddy Theme.command`：从 15 套主题中选择并实时应用
- `WorkBuddy Dream Skin.command`：启动换肤并继续使用上次选择
- `Restore WorkBuddy.command`：恢复原版
- `Dream Skin Editor.command`：打开皮肤编辑器

macOS 启动后同样可使用左侧菜单右边的 17 个胶囊按钮即时切换、使用“自定义”内置编辑器、GitHub 图标和收起拉手。默认读取 `/Applications/WorkBuddy.app`，独立安装目录为 `~/Library/Application Support/WorkBuddyDreamSkin/app`；Release DMG 已内置 Node.js。
