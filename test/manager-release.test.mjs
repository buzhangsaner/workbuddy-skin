...
test('0.6.1 release metadata and manager artifact names stay aligned', async () => {
  ...
  assert.match(renderer, /VERSION = '0\.6\.1'/);
  assert.match(iss, /#define AppVersion "0\.6\.1"/);
  assert.match(plist, /<string>0\.6\.1<\/string>/);
...
test('README leads non-technical users to the manager EXE and DMG', async () => {
  ...
  assert.match(readme, /WorkBuddy-Skin-Manager-Portable-0\.6\.1-x64\.exe/);
  assert.match(readme, /WorkBuddy-Skin-Manager-0\.6\.1-x64\.dmg/);
  assert.match(readme, /WorkBuddy-Skin-Manager-0\.6\.1-arm64\.dmg/);
